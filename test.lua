...
local TARGET_PLACE_ID = 131884594917121 -- replace with your game ID

if game.PlaceId == TARGET_PLACE_ID then
    for _, obj in ipairs(workspace.Debris.WaterDamage:GetDescendants()) do
        if obj:IsA("BasePart") and table.find(partsToDestroy, obj.Name) then
            obj:Destroy()
        end
    end
for _, obj in ipairs(workspace.Debris:GetDescendants()) do
        if obj:IsA("BasePart") and table.find(partsToDestroy, obj.Name) then
            obj:Destroy()
        end
    end
...
-- forward declarations for runtime control
local StartMobAttack
local StartAutoMine
local ApplyConfigRuntimeState
local ResetMobTopCamera
local CleanupFly
local CleanupMineFly
local SetupMineFly
local SetupSafeEffectBypass
...
task.spawn(function()
    task.wait(2)
    if getgenv().EffectBypassEnabled == false then
        return
    end
    local installed = false
    for _ = 1, 20 do
        local ok, result = pcall(SetupSafeEffectBypass)
    ...
    if not installed then
        warn("[EffectBypass] Failed to initialize safe effect bridge")
    end
end)
...
getgenv().EffectBypassEnabled = (getgenv().EffectBypassEnabled ~= false)
getgenv().EffectBypassDisconnectOriginal = (getgenv().EffectBypassDisconnectOriginal ~= false)
getgenv().EffectBypassGuardian = (getgenv().EffectBypassGuardian ~= false)

getgenv().AutoMob = false
getgenv().SelectedMobs = {}
getgenv().AttackDistance = 2
getgenv().FlySpeed = 50
getgenv().MobCamTop = false
getgenv().MobCamTopHeight = 14
...
getgenv().AllowQuestAutoEnable = false


getgenv().AvoidLockedAreas = false

getgenv().MAP_PLACE_IDS = {76558904092080, 129009554587176, 131884594917121, 74414241680540}
...
local EffectBypassState = {
    ...
    effectSignal = nil,
    failCache = {},
    warnedNoGetConnections = false,
}

local function DisconnectUnsafeEffectConnections(effectSignal, keepConnection)
    if typeof(effectSignal) ~= "RBXScriptSignal" then
        return 0
    end
    if not getconnections then
        if not EffectBypassState.warnedNoGetConnections then
            EffectBypassState.warnedNoGetConnections = true
            warn("[EffectBypass] getconnections unavailable (executor limitation); cannot detach original EffectController listeners.")
        end
        return 0
    end
    ...
    pcall(function()
        for _, connection in ipairs(getconnections(effectSignal)) do
            if connection and connection.Connected and connection ~= keepConnection then
...
SetupSafeEffectBypass = function()
    if getgenv().EffectBypassEnabled == false then
        return false
    end

    if EffectBypassState.hooked then
        if EffectBypassState.conn and EffectBypassState.conn.Connected then
            return true
        end
        EffectBypassState.hooked = false
    end
    ...
    local Knit = knitModule and Helpers.SafeRequire(knitModule)
    if not Knit then
        return false
    end

    local okService, EffectService = pcall(function()
        return Knit.GetService("EffectService")
    end)
    if not okService or not EffectService or typeof(EffectService.Event) ~= "RBXScriptSignal" then
        return false
    end

    EffectBypassState.effectSignal = EffectService.Event

    if getgenv().EffectBypassDisconnectOriginal ~= false then
        DisconnectUnsafeEffectConnections(EffectBypassState.effectSignal, nil)
    end

    EffectBypassState.conn = EffectBypassState.effectSignal:Connect(function(effectModule, handlerName, ...)
        if typeof(effectModule) ~= "Instance" or not effectModule:IsA("ModuleScript") then
            return
        end
        ...
        local cacheKey = effectModule:GetFullName() .. "::" .. handlerName
        local lastFail = EffectBypassState.failCache[cacheKey]
        if lastFail and tick() - lastFail < 5 then
            return
        end

        local okRequire, effectTable = pcall(function()
            return require(effectModule)
        end)
        if not okRequire or type(effectTable) ~= "table" then
            EffectBypassState.failCache[cacheKey] = tick()
            return
        end

        local fn = effectTable[handlerName]
    ...
    if getgenv().EffectBypassDisconnectOriginal ~= false then
        DisconnectUnsafeEffectConnections(EffectBypassState.effectSignal, EffectBypassState.conn)
    end

    if getgenv().EffectBypassGuardian ~= false and not EffectBypassState.guardianConn then
        local elapsed = 0
        EffectBypassState.guardianConn = RunService.Heartbeat:Connect(function(dt)
            elapsed = elapsed + dt
            if elapsed < 1 then
                return
            end
            ...
            if getgenv().EffectBypassEnabled == false or getgenv().EffectBypassDisconnectOriginal == false then
                return
            end
            DisconnectUnsafeEffectConnections(EffectBypassState.effectSignal, EffectBypassState.conn)
        end)
...
local MobList = {
    ...
    "Brute Oni","Frostburn Oni","Hellflame Oni","Warlord Oni","Monk Panda","Mountain Ape","Samurai Ape","Savage Ape"
}
...
Helpers.RockDropsSelectedOre = function(rockName)
    local selectedOres = getgenv().SelectedOres
    if not Helpers.HasAnySelection(selectedOres) then return false end
    local ores = RockOreData[rockName]
    if not ores then return false end
    for _, ore in ipairs(ores) do
        if Helpers.IsSelected(ore, selectedOres) then
            return true
        end
    end
    return false
...
Helpers.HasAnySelection = function(selectionList)
    if selectionList == nil then return false end
    if type(selectionList) ~= "table" then
        return selectionList ~= ""
    end
    return next(selectionList) ~= nil
end

Helpers.IsSelected = function(item, selectionList)
    if selectionList == nil then return false end
    if type(selectionList) ~= "table" then
        return item == selectionList
    end

    for k, selected in pairs(selectionList) do
        if item == selected then return true end
        if selected == true and item == k then return true end
    end
    return false
end

Helpers.SelectionToArray = function(selectionList, allowedValues)
    local result = {}
    local seen = {}

    if type(selectionList) ~= "table" then
        if type(selectionList) == "string" and selectionList ~= "" then
            table.insert(result, selectionList)
        end
        return result
    end

    if type(allowedValues) == "table" then
        for _, itemName in ipairs(allowedValues) do
            if Helpers.IsSelected(itemName, selectionList) and not seen[itemName] then
                seen[itemName] = true
    ...
    for key, value in pairs(selectionList) do
        local itemName = nil
...
local Paragraph = ForgeTab:Paragraph({
    Title = "Be aware:",
    Desc = "All these functions can sometimes not work on Xeno or Solara",
    Color = "Grey",
    Locked = false,
})
ForgeTab:Toggle({
    ...
    state.conns.transparent = RunService.RenderStepped:Connect(function()
        ...
        for _, target in ipairs(rayTargets) do
            ...
            for _ = 1, 10 do
                ...
                local shouldHandlePart = part:IsA("BasePart")
                if shouldHandlePart then
                    ...
                    if inRocks then
                        ...
                        if partName == "hitbox" or partName == "ore" then
                            shouldHandlePart = false
                        end
                    end

                    if shouldHandlePart and (inAssets or inRocks) then
...
MiscTab:Button({
    Title = "ESP",
    Desc = "A simple UI to ESP mobs/rocks",
    Callback = function()
--==================================================
-- SAFE RAYFIELD LOAD
--==================================================
...
for _, obj in ipairs(Workspace:GetDescendants()) do
    if obj:IsA("Highlight") and (obj.Name == "MobESP" or obj.Name == "ResourceESP") then
        obj:Destroy()
    elseif obj:IsA("BillboardGui") and (obj.Name == "MobNameTag" or obj.Name == "ResourceNameTag") then
        obj:Destroy()
    end
end
...
local Window = Rayfield:CreateWindow({
    Name = "Toasty ESP: Mobs + Rocks",
    LoadingTitle = "Toasty ESP Loader",
...
local ResourceESPObjects = {}

--==================================================
-- FUNCTIONS
--==================================================
local function isSelected(name, list)
...
local function clearESP(tableToClear)
    for obj, data in pairs(tableToClear) do
        if data.Highlight then data.Highlight:Destroy() end
        if data.NameTag then data.NameTag:Destroy() end
    end
    table.clear(tableToClear)
end
...
local function getDistance(part)
    ...
    return math.floor((part.Position - hrp.Position).Magnitude)
end

--==================================================
-- DISTANCE-BASED COLOR
--==================================================
local function distanceColor(dist, isResource)
    local maxDist = 200
    local t = math.clamp(dist / maxDist, 0, 1)
    
    if not isResource then
        -- Mob: Red (close) -> Green (far)
        local r, g
        if t <= 0.5 then
            r = 255
        ...
        -- Resource: Green (close) -> Red (far)
        local r, g
        if t <= 0.5 then
            g = 255
            r = math.floor(255 * (t / 0.5))
        else
            g = math.floor(255 * (1 - (t - 0.5)/0.5))
            r = 255
        end
        return Color3.fromRGB(r, g, 0)
    end
...
-- ADD ESP FUNCTION
--==================================================
local function addESP(model, tableToStore, isResource)
    if tableToStore[model] then return end
    local primaryPart = model:FindFirstChildWhichIsA("BasePart", true)
    if not primaryPart then return end

    local h = Instance.new("Highlight")
    h.Name = isResource and "ResourceESP" or "MobESP"
    h.FillColor = distanceColor(getDistance(primaryPart), isResource)
    h.OutlineColor = Color3.new(1, 1, 1)
    h.FillTransparency = isResource and 0.6 or 0.4
    h.OutlineTransparency = 0
    h.Adornee = model
    h.Parent = model

    local bp = Instance.new("BillboardGui")
    bp.Name = isResource and "ResourceNameTag" or "MobNameTag"
    bp.Adornee = primaryPart
    bp.Size = isResource and UDim2.new(0,80,0,40) or UDim2.new(0,100,0,50)
...
task.spawn(function()
    while true do
        ...
        for _, data in pairs(ESPObjects) do
            local dist = getDistance(data.Part)
            if dist ~= data.LastValue then
                data.BottomLabel.Text = tostring(dist)
                data.Highlight.FillColor = distanceColor(dist, false)
                data.LastValue = dist
            end
        end
        for _, data in pairs(ResourceESPObjects) do
            local dist = getDistance(data.Part)
            data.Highlight.FillColor = distanceColor(dist, true)
            local val = data.Model:GetAttribute("Health")
            val = val and math.floor(val) or 0
            if val ~= data.LastValue then
                data.BottomLabel.Text = tostring(val)
                data.LastValue = val
            end
        end
    end
...
local MobDropdown = MainTab:CreateDropdown({
    ...
    Callback = function(options)
        ...
        if ESPEnabled then
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") and isSelected(obj.Name, SelectedMobs) then
                    addESP(obj, ESPObjects, false)
                end
            end
        end
    end
...
local ResourceDropdown = MainTab:CreateDropdown({
    ...
    Callback = function(options)
        ...
        clearESP(ResourceESPObjects)
        if ResourceESPEnabled then
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") and isSelected(obj.Name, SelectedResources) then
                    addESP(obj, ResourceESPObjects, true)
                end
            end
        end
    end
...
Workspace.DescendantAdded:Connect(function(obj)
    if ESPEnabled and obj:IsA("Model") and isSelected(obj.Name, SelectedMobs) then
        task.wait()
        addESP(obj, ESPObjects, false)
    end
    if ResourceESPEnabled and obj:IsA("Model") and isSelected(obj.Name, SelectedResources) then
        task.wait()
        addESP(obj, ResourceESPObjects, true)
    end
end)
...
Workspace.DescendantRemoving:Connect(function(obj)
    if ESPObjects[obj] then
        ...
        if ESPObjects[obj].NameTag then ESPObjects[obj].NameTag:Destroy() end
        ESPObjects[obj] = nil
    end
    if ResourceESPObjects[obj] then
        if ResourceESPObjects[obj].Highlight then ResourceESPObjects[obj].Highlight:Destroy() end
        if ResourceESPObjects[obj].NameTag then ResourceESPObjects[obj].NameTag:Destroy() end
        ResourceESPObjects[obj] = nil
    end
end)
...
MobsTab:Slider({
    Title = "Under Mob Distance",
    Desc = "Vertical distance below mob while attacking (higher = safer)",
    Step = 1,
    Flag = "AttackDistance",
    Value = {
        Min = 2,
...
MobsTab:Toggle({
    Title = "Cam On Top Of Mob",
    Desc = "Locks camera above current mob target",
    Value = false,
    Flag = "MobCamTop",
    Callback = function(state)
        getgenv().MobCamTop = state
        if not state and ResetMobTopCamera then
            ResetMobTopCamera()
        end
    end
})
...
MobsTab:Slider({
    Title = "Cam Top Height",
    Desc = "Camera height above mob",
    Step = 1,
    Flag = "MobCamTopHeight",
    Value = {
        Min = 6,
        Max = 40,
        Default = 14,
...
MobsTab:Toggle({
    ...
    Callback = function(state)
        ...
        -- during config load: only set flags, don't start/stop loops yet
        if ConfigState and ConfigState.isLoading then
            return
        end

        if state then
            if not mobConnection then
                StartMobAttack()
            end
            WindUI:Notify({
            ...
            if mobConnection then
                mobConnection:Disconnect()
                mobConnection = nil
            end
            CleanupFly()
            if ResetMobTopCamera then
                ResetMobTopCamera()
            end
            WindUI:Notify({
...
local BossData = {
    ["Golem"] = {
        ...
        ["Asura's Incarnate"] = {
        ActiveDuration = 900,            SpawnInterval = 3600,            Arena = "Island4BossArena",
        RegionName = "Island4BossArena",
    },
...
local function GetSelectedBossName()
    return getgenv().SelectedBoss or "Golem"
end
...
function BossController:GetClosestGolem()
    ...
    for _, mob in pairs(Living:GetChildren()) do
                local baseName = mob.Name:gsub("%d+$", "")         if mob:IsA("Model") and baseName == (getgenv().SelectedBoss or "Golem") then
...
function BossController:IsAnyGolemUsingSkill()
    ...
    for _, mob in pairs(Living:GetChildren()) do
        ...
        if mob:IsA("Model") and baseName == (getgenv().SelectedBoss or "Golem") then
...
function BossController:AreGolemsAlive()
    ...
    for _, mob in pairs(Living:GetChildren()) do
        local baseName = mob.Name:gsub("%d+$", "")
        if mob:IsA("Model") and baseName == selectedBoss then
            local humanoid = mob:FindFirstChildOfClass("Humanoid")
            if humanoid and humanoid.Health > 0 then
                return true
            end
...
function BossController:SpawnGolems()
    ...
    WindUI:Notify({Title = "Spamming Parties", Content = "Spamming " .. self.spamCount .. " " .. selectedBoss .. " parties...", Duration = 3, Icon = "zap"})
    ...
    if setfpscap then setfpscap(1) end
        ...
        for i = 1, self.spamCount do
            coroutine.wrap(function() CreateParty:InvokeServer(selectedBoss) end)()
    ...
    if not bossSpawned then
        WindUI:Notify({Title = "No Boss Spawned", Content = "Waiting 5 minutes before retrying...", Duration = 5, Icon = "clock"})
        ...
        WindUI:Notify({Title = "Retrying", Content = "5 minutes passed, attempting to spawn " .. selectedBoss .. " again...", Duration = 3, Icon = "refresh-cw"})
        return     end
    
    WindUI:Notify({Title = selectedBoss .. " Spawned!", Content = "Starting to fight...", Duration = 3, Icon = "sword"})
end
...
function BossController:MainLoop()
    while self.isActive do
        local selectedBoss = GetSelectedBossName()
        if self:AreGolemsAlive() then
            WindUI:Notify({Title = selectedBoss .. " Found!", Content = "Starting to kill...", Duration = 3, Icon = "sword"})
            
            self:SetNoClip(true)
            self:StartKillLoop()
            print("kill loop started, waiting until selected boss is dead")
            ...
            if not self:AreGolemsAlive() then
...
function BossController:Start()
    ...
    local selectedBoss = GetSelectedBossName()
    WindUI:Notify({Title = "Boss Auto Started", Content = "Starting " .. selectedBoss .. " farming loop...", Duration = 3, Icon = "crown"})
...
BossesTab:Section({
    Title = "Boss Auto (Separate Controller)",
    Icon = "crown",
})

local defaultSelectedBoss = BossList[1] or "Golem"
if table.find(BossList, "Golem") then
    defaultSelectedBoss = "Golem"
end

if not getgenv().SelectedBoss or not table.find(BossList, getgenv().SelectedBoss) then
    getgenv().SelectedBoss = defaultSelectedBoss
end

BossesTab:Dropdown({
    Title = "Select Boss",
    Desc = "Choose which boss to fight",
    Values = BossList,
    Value = getgenv().SelectedBoss,
    Flag = "SelectedBossTarget",
    Callback = function(value)
...
BossesTab:Toggle({
    ...
    Desc = "Waits until 19k HP then kills the boss instantly.",
    Value = false,
...
BossUI.GolemCountParagraph = BossesTab:Paragraph({
    Title = "Bosses in World",
    Desc = "Count: 0 (enable Auto Boss to start tracking)",
})

BossesTab:Toggle({
    Title = "Start Boss Killing Auto",
    Desc = "Spam parties → Reset character → Kill all Bosses (separate controller)",
    Value = false,
    Flag = "GolemBossAuto",
    Callback = function(state)
        if state then
            BossController:Start()
                ...
                while BossController.isActive do
                    ...
                    pcall(function()
                        BossUI.GolemCountParagraph:SetDesc("Count: " .. count .. " Boss(es) alive")
                    end)
                    task.wait(5)
                end
                                pcall(function()
        ...
        while getgenv().AutoQuest do
            ...
            elseif not targets.hasKill then
                ...
                if #validMineTargets > 0 then
                    ...
                    if getgenv().AllowQuestAutoEnable and not getgenv().AutoMine then
...
Helpers.ResolvePotionShopName = function(potionName)
    if type(potionName) ~= "string" or potionName == "" then
        return nil
    end
    ...
    if proximity:FindFirstChild(potionName) then
        return potionName
    end

    local normalizedTarget = string.lower((potionName:gsub("[^%w]", "")))
    local fallback = nil

    for _, obj in ipairs(proximity:GetChildren()) do
        local normalizedName = string.lower((obj.Name:gsub("[^%w]", "")))
        if normalizedName == normalizedTarget then
            return obj.Name
        end
        if not fallback and (string.find(normalizedName, normalizedTarget, 1, true) or string.find(normalizedTarget, normalizedName, 1, true)) then
            fallback = obj.Name
        end
    end

    return fallback or potionName
end
...
local function FlyToPotionShop()
    ...
    local shopHandle = Helpers.GetPotionShopHandle()
    if shopHandle then
...
local function BuyPotionAtShop(potionName, quantity)
    ...
    local resolvedName = Helpers.ResolvePotionShopName(potionName) or potionName
    ...
    local attemptNames = {resolvedName}
    if resolvedName ~= potionName then
        table.insert(attemptNames, potionName)
    end

    local success = false
    local result = nil
    for _, tryName in ipairs(attemptNames) do
        success, result = pcall(function()
            return purchaseRemote:InvokeServer(tryName, maxBuy)
        end)
        if success then
            break
        end
    ...
    if not success then
        warn("[Potions] Failed to buy " .. tostring(potionName) .. ": " .. tostring(result))
        return false
    end

    task.wait(0.15)
    local afterCount = Helpers.GetPotionCount(potionName)
    return afterCount > currentCount
end

local function BuyMultiplePotionsWithFly(potionsList, quantity)
    local potionArray = Helpers.SelectionToArray(potionsList, PotionData.All)
    if #potionArray == 0 then return 0, 0 end
    ...
    for _, potionName in ipairs(potionArray) do
...
PotionsTab:Button({
    ...
    Callback = function()
        local selectedPotions = Helpers.SelectionToArray(PotionBuyState.selected, PotionData.All)
        PotionBuyState.selected = selectedPotions
        ...
        if #selectedPotions == 0 then
            ...
            local bought, failed = BuyMultiplePotionsWithFly(selectedPotions, PotionBuyState.quantity)
...
local function StartAutoHealth()
    ...
    Threads.autoHealth = task.spawn(function()
        while getgenv().AutoHealthPotion do
            ...
            if humanoid and humanoid.Health > 0 and #selectedHealthPotions > 0 then
...
ConfigTab:Button({
    ...
    Callback = function()
        if ConfigState.currentName and ConfigState.currentName ~= "" then
            ...
            if ApplyConfigRuntimeState then
                ApplyConfigRuntimeState()
            end
...
task.spawn(function()
    local success, err = pcall(function()
        local Knit = Helpers.SafeRequire(ReplicatedStorage:WaitForChild("Shared"):WaitForChild("Packages"):WaitForChild("Knit"))
        if not Knit then
            return
        end
        local okController, PlayerController = pcall(function()
...
SettingsTab:Button({
    ...
    Callback = function()
        pcall(function()
            ...
            local okPortal, PortalService = pcall(function()
                return Knit.GetService("PortalService")
            end)
            if not okPortal or not PortalService then
                return
            end
            local result = PortalService:TeleportToIsland("Stonewake's Cross")
...
SettingsTab:Button({
    ...
    Callback = function()
        pcall(function()
            ...
            local result = PortalService:TeleportToIsland("Forgotten Kingdom")
            if type(result) == "table" and result.await then
                result:await()
            end
        end)
...
task.spawn(function()
    ...
    local Knit = Helpers.SafeRequire(Packages:WaitForChild("Knit"))
    local Utils = Helpers.SafeRequire(Shared:WaitForChild("Utils"))
    if not Knit or not Utils then
        warn("[SkipToHammer] Knit/Utils not available")
        return
    end
    
    local okForgeController, ForgeController = pcall(function()
        return Knit.GetController("ForgeController")
    end)
    local okUIController, UIController = pcall(function()
        return Knit.GetController("UIController")
    end)
    local okForgeService, ForgeService = pcall(function()
        return Knit.GetService("ForgeService")
    end)
    if not okForgeController or not okUIController or not okForgeService then
        warn("[SkipToHammer] Required controller/service unavailable")
        return
    end
...
local function UpdateMobTopCamera(mobHRP)
    if not getgenv().MobCamTop or not mobHRP then
        return
    end
    ...
    if (not mobCameraState.active) or mobCameraState.camera ~= cam then
        mobCameraState.active = true
        mobCameraState.camera = cam
        mobCameraState.prevType = cam.CameraType
        mobCameraState.prevSubject = cam.CameraSubject
    end

    cam.CameraType = Enum.CameraType.Scriptable

    local height = tonumber(getgenv().MobCamTopHeight) or 14
    local camPos = mobHRP.Position + Vector3.new(0, height, 0)
    cam.CFrame = CFrame.new(camPos, mobHRP.Position)
end

ResetMobTopCamera = function()
    local cam = workspace.CurrentCamera or mobCameraState.camera
    if cam then
        local function restoreToCharacterCamera()
            local character = LocalPlayer.Character
            local humanoid = character and character:FindFirstChildOfClass("Humanoid")
            ...
            if humanoid then
                ...
                cam.CameraSubject = mobCameraState.prevSubject
            end
        end

        restoreToCharacterCamera()

        -- Retry briefly in case camera/character state updates a frame later.
        task.spawn(function()
            for _ = 1, 10 do
                task.wait(0.05)
    ...
    mobCameraState.active = false
    mobCameraState.camera = nil
    mobCameraState.prevType = nil
    mobCameraState.prevSubject = nil
end
    ...
    mobConnection = RunService.Heartbeat:Connect(function()
        ...
        if mobLastTargetId ~= currentTargetId then
            ...
            mobFlightStartY = hrp.Position.Y
                ...
                local underDistance = tonumber(getgenv().AttackDistance) or 2
        local targetPosition = mobHRP.Position - Vector3.new(0, underDistance, 0)
        ...
        if getgenv().MobCamTop then
            UpdateMobTopCamera(mobHRP)
        else
            ResetMobTopCamera()
        end
            ...
            NotifService.Notify:Connect(function(data)
                if type(data) == "table" and data.Text then
                    local text = string.lower(data.Text)
                            ...
                            local okAttr, lastHitPlayer = pcall(function()
                                return currentRockTarget:GetAttribute("LastHitPlayer")
                            end)
                            
                                                        if okAttr and lastHitPlayer and lastHitPlayer ~= LocalPlayer.Name then
...
local function IsRockBeingMined(rock)
    if typeof(rock) ~= "Instance" or not rock.Parent then return false end

    local okPlayer, lastHitPlayer = pcall(function()
        return rock:GetAttribute("LastHitPlayer")
    end)
    local okTime, lastHitTime = pcall(function()
        return rock:GetAttribute("LastHitTime")
    end)
    if not okPlayer or not okTime then
        return false
    end
...
function GetAttackingMobNearPosition(position, range)
...
function IsMarkedThreatValid()
    if typeof(markedThreat) ~= "Instance" then return false end
...
function ClearMarkedThreat()
    markedThreat = nil
end

function KillThreat(threatMob)
    if typeof(threatMob) ~= "Instance" or not threatMob.Parent then
        return false
    end
    ...
    local defendVelocity = nil
    local defendGyro = nil
    local defendConnection = nil

    local created, createErr = pcall(function()
        defendVelocity = Instance.new("BodyVelocity")
        ...
        defendGyro = Instance.new("BodyGyro")
        defendGyro.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
        defendGyro.P = 10000
        defendGyro.D = 100
        defendGyro.Parent = hrp
    end)
    if not created or not defendVelocity or not defendGyro then
        warn("[AutoMine] KillThreat setup failed: " .. tostring(createErr))
    ...
    defendConnection = RunService.Heartbeat:Connect(function()
        ...
        if not mobHumanoid or mobHumanoid.Health <= 0 or not mobHRP then
            finished = true
            return
        end

        local targetPosition = mobHRP.Position - Vector3.new(0, DEFEND_ATTACK_DISTANCE + DEFEND_DEPTH, 0)
        local direction = targetPosition - currentHrp.Position
        local distance = direction.Magnitude
        ...
        defendGyro.CFrame = CFrame.new(currentHrp.Position, mobHRP.Position)
    ...
    while not finished do
        if tick() - startTime > (maxKillTime + 1) then
            break
        end
        task.wait(0.1)
    end
    ...
    pcall(function()
        local resetCharacter = LocalPlayer.Character
        local resetHumanoid = resetCharacter and resetCharacter:FindFirstChildOfClass("Humanoid")
        if resetHumanoid then
            resetHumanoid.PlatformStand = false
        end
    end)
...
function ShouldDefend(rockPosition)
    if not getgenv().DefendWhileMining then
        return false, nil
    end
    ...
    local checkPosition = rockPosition
    if typeof(checkPosition) ~= "Vector3" then
        if not hrp then
            return false, nil
        end
        checkPosition = hrp.Position
    end

    local range = tonumber(getgenv().DefendRange) or 20
    local threat = GetAttackingMobNearPosition(checkPosition, range)
...
function RockHasDesiredOre(rock)
    if typeof(rock) ~= "Instance" or not rock.Parent then
        return nil
    end

    local selectedOres = getgenv().SelectedOres
    if not Helpers.HasAnySelection(selectedOres) then
        return nil
    end

    local foundAnyOreInfo = false
    for _, child in ipairs(rock:GetDescendants()) do
        local okAttr, oreAttr = pcall(function()
            return child:GetAttribute("Ore")
        end)
        if okAttr and type(oreAttr) == "string" then
            foundAnyOreInfo = true
            if Helpers.IsSelected(oreAttr, selectedOres) then
                return true
            end
        end
    end

    if not foundAnyOreInfo then
        return nil
    end
...
function GetClosestSelectedRock()
    local Rocks = workspace:FindFirstChild("Rocks")
    ...
    local selectedMineAreas = getgenv().SelectedMineAreas or {}
    local selectedRocks = getgenv().SelectedRocks or {}
    local avoidPlayers = getgenv().AvoidPlayersWhileMining == true
    local mineMode = getgenv().MineMode or "Rocks"
    ...
    local function IsRockSkipped(rock)
        ...
        return skippedRocks[rock] ~= nil
    end

    local function IsAreaSelected(areaName)
        if not getgenv().FilterMineByArea then return true end
        if not Helpers.HasAnySelection(selectedMineAreas) then return true end
        return Helpers.IsSelected(areaName, selectedMineAreas)
    end
    ...
    local function GetRockPart(rock)
        ...
        if rock:IsA("Model") then
            return rock.PrimaryPart or rock:FindFirstChild("Hitbox") or rock:FindFirstChildOfClass("BasePart")
        end
    ...
    for _, areaFolder in pairs(Rocks:GetChildren()) do
        if areaFolder:IsA("Folder") and IsAreaSelected(areaFolder.Name) then
            for _, spawnLocation in pairs(areaFolder:GetChildren()) do
                for _, rock in pairs(spawnLocation:GetChildren()) do
                    local shouldMine = false
                    if mineMode == "Rocks" then
                        shouldMine = Helpers.IsSelected(rock.Name, selectedRocks)
                    ...
                    if not skipRock and IsRockSkipped(rock) then
                        skipRock = true
                    end
                    if not skipRock and avoidPlayers and (IsRockBlacklisted(rock) or IsRockBeingMined(rock)) then
                        skipRock = true
                    end

                    local rockHealth = nil
                    if not skipRock then
                        local okHealth, healthAttr = pcall(function()
                            return rock:GetAttribute("Health")
                        end)
                        rockHealth = okHealth and tonumber(healthAttr) or nil
                        if rockHealth ~= nil and rockHealth <= 0 then
                            skipRock = true
                        end
                    end
                    ...
                    local rockPart = nil
                    if not skipRock then
                        rockPart = GetRockPart(rock)
                        if not rockPart then
                            skipRock = true
                        end
                    end
                    if not skipRock and avoidPlayers and rockPart and Helpers.IsPlayerNearRock(rockPart, getgenv().PlayerAvoidDistance) then
                        skipRock = true
                    end

                    if not skipRock and rockPart then
                        local distance = (hrp.Position - rockPart.Position).Magnitude
                        local compareHealth = rockHealth or math.huge
                        local isBetter = false

                        if useSpecificPriority then
                            local itemPriority = GetItemPriority(rock.Name)
                        ...
                        elseif priority == "Weakest" then
                            if bestValue == nil or compareHealth < bestValue then
                                bestValue = compareHealth
                                isBetter = true
                            end
                        elseif priority == "Strongest" then
                            if bestValue == nil or compareHealth > bestValue then
...
function GetAnyRockInSelectedAreas()
    local Rocks = workspace:FindFirstChild("Rocks")
    if not Rocks then return nil end
        ...
        if areaFolder:IsA("Folder") and IsAreaSelected(areaFolder.Name) then
            for _, spawnLocation in pairs(areaFolder:GetChildren()) do
                for _, rock in pairs(spawnLocation:GetChildren()) do
                    ...
                    local health = okHealth and tonumber(healthAttr) or nil
...
function IsRockValid(rock)
    ...
    if not (rock:IsA("Model") or rock:IsA("BasePart")) then
        return false
    end

    local ok, healthAttr = pcall(function()
        return rock:GetAttribute("Health")
    end)
    if not ok then
        return false
    ...
    local health = tonumber(healthAttr)
    if health ~= nil and health <= 0 then
        return false
    end
    return true
...
SetupMineFly = function(hrp)
    ...
    if not ok then
        warn("[AutoMine] SetupMineFly failed: " .. tostring(err))
        pcall(function()
            if mineBodyVelocity then mineBodyVelocity:Destroy() end
        end)
        pcall(function()
...
function StartAutoMine()
    ...
    mineConnection = RunService.Heartbeat:Connect(function()
        ...
        local targetPosition = rockPart.Position + Vector3.new(0, -depthOffset, mineDistance)
                ...
                if not mineBodyVelocity or not mineBodyVelocity.Parent or not mineBodyGyro or not mineBodyGyro.Parent then
            SetupMineFly(hrp)
            if humanoid then
                humanoid.PlatformStand = true
            end
        ...
        if not mineBodyVelocity or not mineBodyGyro then
            return
        end
        ...
        if antiGoblin then
            ...
            if distance > 1 then
                ...
                if shouldDefend and threat then
                    ...
                    if not killOk then
                        warn("[AutoMine] KillThreat runtime error: " .. tostring(killErr))
                    end
                    
                                        currentRockTarget = rockToResume
                    isDefending = false
                    
                                        local resumeCharacter = LocalPlayer.Character
                    local resumeHrp = resumeCharacter and resumeCharacter:FindFirstChild("HumanoidRootPart")
                    local resumeHumanoid = resumeCharacter and resumeCharacter:FindFirstChildOfClass("Humanoid")
                    if resumeHrp then
                        SetupMineFly(resumeHrp)
                        if resumeHumanoid then
                            resumeHumanoid.PlatformStand = true
                        end
                    end
...
function ApplyConfigRuntimeState()
    -- Sync AutoMob loop with current flag
    if getgenv().AutoMob then
        if not mobConnection then
            StartMobAttack()
        end
    ...
    -- Sync AutoMine loop with current flag
    if getgenv().AutoMine then
        if not mineConnection then
            StartAutoMine()
        end
        ...
        if CleanupMineFly then
            CleanupMineFly()
        end
    end

    -- Sync AutoQuest with current flag
    if getgenv().AutoQuest then
        if QuestsUI and QuestsUI.autoQuestToggle then
            QuestsUI.autoQuestToggle:Set(true)
            StartAutoQuest(QuestsUI.autoQuestToggle)
        end
    else
        if StopAutoQuest then
            StopAutoQuest()
        end
        if QuestsUI and QuestsUI.autoQuestToggle then
            QuestsUI.autoQuestToggle:Set(false)
            if QuestsUI.progressParagraph then
                QuestsUI.progressParagraph:SetDesc("Auto Quest disabled")
            end
        end
